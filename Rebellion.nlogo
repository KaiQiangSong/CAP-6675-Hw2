...
cops-own [
  undercover?         ; Whether a cop is undercover cop or not. (if 0, the normal cop, if 1 the undercover cop)
]
...
to setup
  ...
  create-cops round (initial-cop-density * .01 * count patches) [
    ...
    set undercover? (random-float 100 <= undercover-ratio)
    display-cop
  ]
...
to-report estimated-arrest-probability
  let c count cops-on neighborhood with [
    all? cops-here [not undercover?]
  ]
